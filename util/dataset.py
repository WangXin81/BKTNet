...
class SemData(Dataset):
    def __init__(self, split='train', data_root=None, data_list=None, \
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)  # 创建目录，如果已存在也不会报错
        ...
        if not os.path.exists(path_np_data_list):
            ...
            if os.path.exists(path_np_data_list) and os.path.exists(path_np_supp_list):
                print('[{}] 文件保存成功: {} 和 {}'.format(split, path_np_data_list, path_np_supp_list))
            else:
                print('[{}] 文件保存失败! 请检查路径和权限。'.format(split))   
        else:
